...
// IntKey is a type constraint for values that can be used as keys in IntMap
...
// IntMap is a hashmap where the keys are some any integer type.
type IntMap[K IntKey, V any] struct {
...
func NewIntMap[K IntKey, V any](capacity int) *IntMap[K, V] {
	return &IntMap[K, V]{
...
func (m *IntMap[K, V]) Get(key K) (V, bool) {
	if key == K(0) {
		if m.hasZeroKey {
...
func (m *IntMap[K, V]) Put(key K, val V) {
	if key == K(0) {
		if !m.hasZeroKey {
			m.size++
		}
...
func (m *IntMap[K, V]) ForEach(f func(K, V)) {
	if m.hasZeroKey {
		f(K(0), m.zeroVal)
	}
	forEach64(m.data, f)
...
func (m *IntMap[K, V]) sizeThreshold() int {
	return int(math.Floor(float64(len(m.data)) * fillFactor64))
}

func (m *IntMap[K, V]) startIndex(key K) int {
	return phiMix64(int(key)) & (len(m.data) - 1)
}

func (m *IntMap[K, V]) nextIndex(idx int) int {
...
func (m *IntMap[K, V]) Del(key K) bool {
	if key == K(0) {
		if m.hasZeroKey {
			m.hasZeroKey = false
			m.size--
...
func (m *IntMap[K, V]) shiftKeys(idx int) int {
